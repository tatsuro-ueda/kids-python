...
| プライバシー | GA4（子ども向け設定） | 下記プライバシーポリシー参照 |

### アナリティクス・プライバシーポリシー

| ポリシー | 内容 | 根拠 |
|---------|------|------|
| EU圏は非トラッキング | EU圏からのアクセスにはGAタグを出力しない | GDPR対応。geoip-liteで国判定しサーバー側で制御 |
| 子ども向け設定 | `ad_storage: denied`, `ad_personalization: denied` | COPPA対応。対象年齢6-12歳のため広告パーソナライズを無効化 |
| 測定IDの管理 | 環境変数 `GA_MEASUREMENT_ID` で管理 | 未設定時はGAタグを出力しない（開発環境ではGA無効） |
